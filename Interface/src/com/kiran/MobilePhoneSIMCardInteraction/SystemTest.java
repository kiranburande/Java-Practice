...
public class SystemTest {
	...
	public static void main(String[] args) 
	{
		...
		mobilePhone.insertSim(jio);
		mobilePhone.removeSim(jio);
		mobilePhone.makeCall(jio);
		mobilePhone.sendTextMsg(jio);
		
		System.out.println("\n------------------------------------\n");
		
		mobilePhone.insertSim(airtel);
		mobilePhone.removeSim(airtel);
		mobilePhone.makeCall(airtel);
		mobilePhone.sendTextMsg(airtel);
		
		System.out.println("\n------------------------------------\n");
		
		mobilePhone.insertSim(bsnl);
		mobilePhone.removeSim(bsnl);
		mobilePhone.makeCall(bsnl);
		mobilePhone.sendTextMsg(bsnl);
...
You are tasked with implementing a simulation of a mobile phone interacting with different SIM cards. The goal is to create a system where mobile phones
 can insert, remove, and use SIM cards from different providers.
...
-> Jio, Airtel, and BSNL Classes: These classes should implement the SIMCardInterface. Each class represents a SIM card from a different
 network provider (Jio, Airtel, and BSNL) and should have provider-specific details.
...
-> MobilePhone Class: This class should implement the MobilePhoneInterface. It should have a slot for inserting a SIM card and methods for making calls
 and sending texts.

Tasks:
----------

-> Implement the SIMCardInterface and the classes Jio, Airtel, and BSNL that implement this interface. Include methods for getting phone number,
 network provider, and activating/deactivating the SIM card.

-> Implement the MobilePhoneInterface and the MobilePhone class that implements this interface. The MobilePhone class should allow inserting and
 removing SIM cards, making calls, and sending texts.
